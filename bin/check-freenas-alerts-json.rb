...
# Check JSON
#
class CheckFreenasAlerts< Sensu::Plugin::Check::CLI
  option :url,
    short: '-u URL'
  option :host,
    short: '-h HOST'
  ...
  def usage_summary
    "Critical: #{@num_critical}, Warnings: #{@num_warning}, OKs: #{@num_ok}"
  end
  ...
  def run
    ...
    critical usage_summary if @num_critical > 0 && config[:criticals]
    warning usage_summary if @num_warning > 0 && config[:warnings]
    ok usage_summary
  end
  ...
  def acquire_resource
    ...
    alerts.each do | alert |
      if alert['level'] == 'WARN'
        @num_warning += 1
      elsif alert['level'] == 'CRITICAL'
        @num_critical += 1
      elsif alert['level'] == 'OK'
        @num_ok += 1
      end
    end
  end
